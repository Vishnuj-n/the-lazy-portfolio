  ...
  <section v-if="projects.length" id="projects" class="px-6 md:px-8 py-16 md:py-20 max-w-5xl mx-auto" aria-labelledby="tier-two-title">
    ...
    <div class="flex flex-wrap gap-4 justify-center">
      <div
        ...
        :key="p.id || p.repoName || p.title || i"
        data-testid="project-card"
        :style="p.isPlaceholder ? 'background: transparent; border: 1px dashed rgba(255,255,255,0.25); border-radius: var(--radius); padding: 24px;' : 'background: var(--card); border: 1px solid var(--border); border-radius: var(--radius); padding: 24px; transition: border-color 0.15s; cursor: pointer;'"
        class="hover:border-[rgba(200,245,66,0.2)] group w-full md:w-auto md:flex-[1_1_calc(50%-0.5rem)]"
        @click="!p.isPlaceholder && $router.push('/project/' + (p.repoName || p.title).toLowerCase())"
      >
        <template v-if="p.isPlaceholder">
          <div class="flex items-start justify-between gap-4 mb-3">
            <div>
              ...
              <h3 style="font-family: var(--font-display); font-size: 18px; font-weight: 600; color: var(--muted-foreground); opacity: 0.7;">
                [Empty Project Slot]
              </h3>
            </div>
          </div>
          <p style="font-family: var(--font-sans); font-size: 13px; line-height: 1.6; color: var(--muted-foreground); opacity: 0.4;">
            Testing 2-column wrapping symmetry for projects section.
          </p>
        </template>
        <template v-else>
          <div class="flex items-start justify-between gap-4 mb-3">
